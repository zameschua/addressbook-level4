...
* Change command word for mass emailing to `email` instead of `mass` as it is more intuitive.
...
Upon entering a valid `add` command, CYNC will create a new `Person` object by creating  which comprises with the as seen in the model diagram below: 

The person object will then be stored in the Storage locally.
