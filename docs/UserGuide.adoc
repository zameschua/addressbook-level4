...
[NOTE]
====
If you made any changes to a person's information and the person's detailed information was showing in the panel before you made the changes,
you have to refresh by entering the command `select (person's index)` again once you are done with editing the information.
====
// end::select[]
...
Please note that the event time has to be in 24hrs format, and the date format is Year-Month-Day. e.g. `16:00` for 4pm +
