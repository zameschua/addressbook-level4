...
Derived from the word " Sync ", CYNC aims to keep customers and businesses in sync. CYNC ensures that customers are businesses' main priority
by storing customers' information and tracking their activities. CYNC also focuses on efficiency thus its
Command Line Interface (CLI) allows users to add or retrieve customer's data quickly.
...
Opens up google calendar in the browser panel +
Format: `calendar`

[TIP]
Instead of a calendar, a google login page will be shown every time the application is restarted. Login to proceed to the calendar.

The UI should appear as such:

image::Calendar.png[width="790"]

=== Adding a event to the calendar

Adds an event to the google calendar linked +
...
=== Mass Emailing : `mass`
Allows user to customise and send out email to all customers or to specific tagged customers. +
Format: `mass` or `mass KEYWORD` +
...
* *Select* : `select INDEX` +
e.g.`select 2`
* *Mass Emailing*: `mass` / `mass KEYWORD` +
e.g `mass friends`
* *History* : `history`
* *Undo* : `undo`
* *Redo* : `redo`
