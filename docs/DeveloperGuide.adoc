...
* consist of Gmail API : Authenticate user's Gmail and send out emails
...
an update to `CalendarApi` for google calendar to add the event. If the calendar event is added successfully, an `event` is
