...
image::ListAllTagsMechanism.png[width="800"]
...
_Figure 5.4.3: Empty `browserPlaceholder` on startup_
...
_Figure 5.4.4 List of tags displayed to the right of person list_
...
**Cons:** This violates the YAGNI principle, as the BrowserPanel's other functionalities are not actually used in user execution. +
...
**Pros:** Since `BrowserPanel` essentially only has one function that is seemingly trivial, it may make sense just to totally remove it.
 This removes the amount of code that we have and also reduces the possiblity of bugs. +
