...
The `mass` command allows the user to send emails to all or certain groups of customers at once.
...
On entering an `mass` command, the `emailPanel` will appear in the MainDisplayPanel area. 
with the desired emails of customers extracted from the relavent persons and pre-filled, as demonstrated in the diagram below.
