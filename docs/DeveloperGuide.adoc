...
1. `email all` : get everyone in CYNC and store their emails in an array.

2.`email KEYWORD [MORE KEYWORDS]`: get all the required persons and store their emails in an array.
...
On entering an `email` command, the `emailPanel` will appear in the MainDisplayPanel area. 
