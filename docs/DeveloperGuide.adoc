...
so that whatever you've written can be easily understood by your peers.
...
image::command-prediction-panel-event-diagram.png[width="800"]
_Figure 4.4.2 : Event Diagram of the Command Prediction Panel_
...
**Alternative 2** (to implement in V1.5): A `CommandManager` will keep track of all existing commands.
...
The List All Tags mechanism is facilitated by the `EventsCenter`, in which it receives an event from `listalltags` command
...
image::CyncStartUp.png[width="800"]
